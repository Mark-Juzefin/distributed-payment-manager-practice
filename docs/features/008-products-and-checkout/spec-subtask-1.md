...
type Repo interface {
    // Create inserts a new product. Returns ErrSlugConflict on (merchant_id, slug) violation.
    Create(ctx context.Context, p *Product) error

    // GetByID returns the product if it belongs to merchantID; ErrNotFound otherwise
    // (used by handlers — they translate to 404, also for cross-merchant isolation).
    GetByID(ctx context.Context, merchantID string, id uuid.UUID) (*Product, error)

    // List with cursor pagination + status filter (StatusFilter zero value = no filter).
    List(ctx context.Context, merchantID string, filter ListFilter) ([]*Product, *Cursor, error)

    // UpdateInfo updates I1 fields (name, description) — always allowed якщо not archived.
    UpdateInfo(ctx context.Context, merchantID string, id uuid.UUID, patch InfoPatch) error

    // UpdatePricing updates I2 fields (slug, price, currency).
    // Returns ErrFieldsLocked if first_purchased_at IS NOT NULL.
    // Atomic conditional UPDATE — no separate SELECT needed.
    UpdatePricing(ctx context.Context, merchantID string, id uuid.UUID, patch PricingPatch) error

    // SetStatus implements I3 transitions. Idempotent.
    SetStatus(ctx context.Context, merchantID string, id uuid.UUID, status Status) error

    // MarkPurchased sets first_purchased_at = now() якщо NULL. Idempotent.
    // Called by /purchase (Subtask 2) на першій успішній покупці.
    // Визначаємо contract тут — тести Subtask 1 використовують для setup frozen state.
    MarkPurchased(ctx context.Context, id uuid.UUID) error
}
```

Service layer розшарує `Update(merchantID, id, updateRequest)` → внутрішньо викликає `UpdateInfo` для I1 полів + `UpdatePricing` для I2 полів. Якщо PATCH містить тільки I1 — `UpdatePricing` не викликається, freeze check не відбувається.
