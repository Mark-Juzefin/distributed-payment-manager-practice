...
func (c *DisputeMessageController) HandleMessage(ctx context.Context, key, value []byte) error {
	...
	c.logger.DebugCtx(ctx, "Processing dispute message: event_id=%s key=%s type=%s",
	...
	if err := c.service.ProcessChargeback(ctx, webhook); err != nil {
		...
		if errors.Is(err, dispute.ErrEventAlreadyStored) {
			c.logger.InfoCtx(ctx, "Duplicate dispute event ignored: event_id=%s user_id=%s order_id=%s provider_event_id=%s",
				env.EventID, webhook.UserID, webhook.OrderID, webhook.ProviderEventID)
			return nil
		}

		c.logger.ErrorCtx(ctx, "Failed to process chargeback webhook: event_id=%s user_id=%s order_id=%s error=%v",
			env.EventID, webhook.UserID, webhook.OrderID, err)
		return err
	}

	c.logger.InfoCtx(ctx, "Chargeback webhook processed: event_id=%s user_id=%s order_id=%s status=%s",
