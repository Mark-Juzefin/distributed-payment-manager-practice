...
type DisputeService struct {
	...
	txEventStore  func(tx postgres.Executor) eventstore.Store
...
func (s *DisputeService) ProcessChargeback(ctx context.Context, webhook ChargebackWebhook) error {
	...
	err := s.transactor.InTransaction(ctx, pgx.RepeatableRead, func(tx postgres.Executor) error {
		...
		if dispute == nil {
			...
			if err := s.writeEvent(ctx, txEvents, eventstore.AggregateDispute, actualDisputeData.ID,
...
func (s *DisputeService) UpsertEvidence(ctx context.Context, disputeID string, upsert EvidenceUpsert) (*Evidence, error) {
	...
	err := s.transactor.InTransaction(ctx, pgx.RepeatableRead, func(tx postgres.Executor) error {
		...
		if err := s.writeEvent(ctx, txEvents, eventstore.AggregateDispute, disputeID,
...
func (s *DisputeService) Submit(ctx context.Context, disputeID string) error {
	...
	err := s.transactor.InTransaction(ctx, pgx.RepeatableRead, func(tx postgres.Executor) error {
		...
		now := time.Now()
		d.SubmittedAt = &now
		d.SubmittingId = &res.ProviderSubmissionID
...
func (s *DisputeService) writeEvent(ctx context.Context, store eventstore.Store, aggregateType eventstore.AggregateType, aggregateID, eventType, idempotencyKey string, payload json.RawMessage) error {
	_, err := store.CreateEvent(ctx, eventstore.NewEvent{
	...
	if errors.Is(err, eventstore.ErrEventAlreadyStored) {
		return nil
	}
	if err != nil {
