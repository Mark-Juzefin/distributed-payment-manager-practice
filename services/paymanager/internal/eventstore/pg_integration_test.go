...
func TestCreateEvent_Success(t *testing.T) {
	...
	err := pool.SandboxTransaction(ctx, func(tx postgres.Executor) error {
		store := eventstore.NewPgEventStore(tx, pool.Builder)

		newEvent := eventstore.NewEvent{
			AggregateType:  eventstore.AggregateOrder,
		...
		assert.Equal(t, eventstore.AggregateOrder, created.AggregateType)
...
func TestCreateEvent_IdempotencyConstraint(t *testing.T) {
	...
	tests := []struct {
		name                 string
		firstEvent           eventstore.NewEvent
		duplicateEvent       eventstore.NewEvent
		...
		},
		{
			...
			duplicateEvent: eventstore.NewEvent{
				AggregateType:  eventstore.AggregateDispute, // Different aggregate type
		...
		t.Run(tt.name, func(t *testing.T) {
			err := pool.SandboxTransaction(ctx, func(tx postgres.Executor) error {
				...
				if tt.expectDuplicateError {
					require.Error(t, err)
					assert.True(t, errors.Is(err, eventstore.ErrEventAlreadyStored),
