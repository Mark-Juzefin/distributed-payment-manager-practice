...
func (s *OrderService) ProcessOrderUpdate(ctx context.Context, update OrderUpdate) error {
	err := s.transactor.InTransaction(ctx, pgx.RepeatableRead, func(tx postgres.Executor) error {
		...
		if err := s.writeEvent(ctx, txEvents, eventstore.AggregateOrder, update.OrderId,
...
func (s *OrderService) UpdateOrderHold(ctx context.Context, orderID string, request HoldRequest) (*HoldResponse, error) {
	...
	err := s.transactor.InTransaction(ctx, pgx.RepeatableRead, func(tx postgres.Executor) error {
		...
		if err := s.writeEvent(ctx, txEvents, eventstore.AggregateOrder, orderID,
...
func (s *OrderService) writeEvent(ctx context.Context, store eventstore.Store, aggregateType eventstore.AggregateType, aggregateID, eventType, idempotencyKey string, payload json.RawMessage) error {
